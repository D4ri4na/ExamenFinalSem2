...
def migrate_data():
    ...
    try:
        ...
        mysql_db_exists_query = f"SHOW DATABASES LIKE '{db_name}';"
        mysql_db_exists = mysql_conn.execute_query(mysql_db_exists_query)

        # Check if the database exists in SQL Server
        mssql_db_exists_query = f"SELECT * FROM sys.databases WHERE name = '{db_name}';"
        mssql_db_exists = mssql_conn.execute_query(mssql_db_exists_query)

        # If the database exists in both MySQL and SQL Server, do not execute anything
        if mysql_db_exists and mssql_db_exists:
            print(f"Database '{db_name}' already exists in both MySQL and SQL Server. No action taken.")
        else:
            # If the database does not exist in both MySQL and SQL Server, execute the migration
            if not mysql_db_exists:
                print(f"Database '{db_name}' does not exist in MySQL. No action taken.")
            elif not mssql_db_exists:
                print(f"Database '{db_name}' does not exist in SQL Server. Creating the database and migrating tables.")
                ...
                for table in tables:
                    ...
                    if not table_exists:
                        ...
                        # Add the 'modification' column to the table
                        add_modification_column_query = f"ALTER TABLE {table_name} ADD modification DATETIME DEFAULT GETDATE();"
                        mssql_conn.execute_update(add_modification_column_query)
